...
- [Agentage Memory](https://memory.agentage.io/mcp) - 🆕 **公式 MCP Registry**（`io.agentage/memory`）—— すべての AI（Claude / Cursor / ChatGPT）が共有・読み書き可能なファイルベースのメモリ。ローカルでマークダウンとして同期されエクスポート可能。リモート Streamable HTTP (OAuth 2.1 + PKCE + Dynamic Client Registration)。6つのツール（`memory__search/read/write/edit/list/delete`）を提供。[ドキュメント](https://agentage.io/blog/mcp-endpoint-is-live)。
...
- [Databricks Genie One](https://www.databricks.com/blog/introducing-genie-one-genie-ontology-and-genie-agents) - 🆕 **2026-06-16（Data + AI Summit）**。Databricks が発表したエージェント型「データ同僚」。構造化・非構造化データを横断して作業を自動オーケストレーションし、新しい **Genie Ontology**（組織全体のナレッジグラフ）に基づき Unity Catalog でガバナンス。Genie Agents と Genie App Builder を同梱、社内テストで初回正答率 84.5%。
...
- [Meta Business Agent](https://about.fb.com/news/2026/06/meta-business-agent/) - 🆕 **2026-06-03（グローバル展開）**。Meta が WhatsApp・Instagram・Messenger 向けに展開する AI ビジネスエージェント。問い合わせ対応、カタログ商品の提案、予約受付、リードの選別、成約までを担い、必要に応じて人間へ引き継ぐ。すでに 100 万以上のビジネスが利用。従量課金の **Meta Business Agent Platform** では企業が独自エージェントを構成し Shopify / Zendesk / Shopee と連携可能。
...
- [Prismix](https://prismix.dev) - 75以上の AI サービス（OpenAI、Anthropic、Cursor など）のステータスをリアルタイム監視し、メール/Webhook でアラート。70以上のソースから AI ニュースを集約。500以上のサーバーを収録する MCP サーバーディレクトリ。
