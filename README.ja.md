...
- [YYLO](https://github.com/yylo-dev/yylo) - 🆕 ⚠️ **Unverified**（GitHub 57 stars；npm `@yylo/cli`；チーム自己申告）。コーディングエージェント向けのコマンドラインオーケストレーター — 再利用可能なワークフローとレシート付きリポジトリ変更を提供：`task start` がタスクごとに専用ブランチ/worktree を作成し、マージキューが型付きタスク・検証・マージ・リリース準備の境界でリスクベースレビューを管理。MIT。![GitHub stars](https://img.shields.io/badge/dynamic/json?label=Stars&query=%24.stargazers_count&url=https%3A%2F%2Fapi.github.com%2Frepos%2Fyylo-dev%2Fyylo&color=yellow&logo=github&logoColor=white&style=flat&cacheSeconds=300)
