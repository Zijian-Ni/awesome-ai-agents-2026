...
- [VibeKit](https://vibekit.bot) - 🆕 Mobile-first builder where every app gets its own persistent AI coding agent that builds it, hosts it on a live URL, and keeps iterating — driven from your phone or the CLI, so it keeps working after the first prototype (no machine to keep awake; survives a dropped connection). BYOK (Claude/OpenAI) or pay-as-you-go; native iOS app. Closed-source SaaS. (Distinct from the superagent-ai/vibekit sandbox SDK.)
