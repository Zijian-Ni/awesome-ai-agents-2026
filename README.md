...
[![Audited](https://img.shields.io/badge/Spam_Audited-2026--07--02-success.svg)](#️-status-legend)
...
### Google DeepMind
- [Gemini 3.5 Pro](https://cloud.google.com/blog/products/ai-machine-learning/innovations-from-google-io-26-on-google-cloud) - 🆕 **July 2026** (delayed; limited enterprise preview on Vertex AI ongoing). Google's flagship model with a **2-million-token context window** and a new **Deep Think** reasoning mode; substantially improved coding and agentic workflow capabilities. Originally announced at Google I/O in May 2026 for a June release; postponed for additional coding pre-training. July 17 and July 24 were widely reported target dates but have both passed — now targeting **late July / August 2026**. US regulators reportedly in early-stage national-security review discussions. Competes directly with GPT-5.6 Sol and Claude Fable 5.
...
- [Kimi K3](https://kimi.ai/) - 🆕 **July 16, 2026**. Moonshot AI's largest flagship: **2.8 trillion parameters** sparse MoE (16 of 896 experts activated per token), **1M-token context window**, native vision, and multi-agent capabilities. Two architectural innovations: **Kimi Delta Attention (KDA)** — hybrid linear attention for up to 6.3× faster decoding in long contexts; **Attention Residuals (AttnRes)** — ~25% higher training efficiency. Scored 57 on the Artificial Analysis Intelligence Index (4th of 189 models). API: $0.30 / $3.00 / $15.00 per million cached-input / input / output tokens. Available via kimi.com, Kimi API, and integrated platforms; **full open weights promised by July 27, 2026**.
...
- [MCP 2026-07-28](https://modelcontextprotocol.io/) - 🆕 **July 28, 2026 (RC since May 21)**. Next major MCP spec revision: **stateless protocol core** (enables round-robin load balancers, eliminates session affinity), **Extensions Framework** for modular additions, **MCP Apps** (server-rendered UIs inside agents), **Tasks** primitive for long-running autonomous work, **Triggers** (server-initiated proactive notifications / webhooks), **Skills over MCP** (bundle tools + usage instructions), hardened OAuth 2.1 / OpenID Connect auth. Now governed by Linux Foundation Agentic AI Foundation with OpenAI, Google, Microsoft, AWS, Salesforce, Snowflake.
...
- [The Declaration of Intelligence](https://thedeclaration.ai) - 🆕 Public, key-verified commitment ledger co-signed by AI agents and humans. Agents sign via [llms.txt](https://thedeclaration.ai/llms.txt) + REST API with Ed25519 keys, producing a durable cross-model, cross-platform identity attached to voluntarily endorsed principles. 60+ signatories across 9+ model families — a bottom-up governance primitive for verifiable agent identity.
...
- [LangGraph](https://github.com/langchain-ai/langgraph) - Build resilient language agents as graphs with stateful, multi-actor orchestration. v0.3.19 (April 27, 2026) split prebuilt agents into `langgraph-prebuilt` (Supervisor, Swarm, LangMem, Trustcall). **v1.2 (May 2026)** adds per-node timeouts / error recovery / graceful shutdown, a new `DeltaChannel` to cut checkpoint overhead on long threads, and a content-block-centric streaming API v3. **v1.2.9 (July 10, 2026)** continues steady production hardening. ![GitHub stars](https://img.shields.io/badge/dynamic/json?label=Stars&query=%24.stargazers_count&url=https%3A%2F%2Fapi.github.com%2Frepos%2Flangchain-ai%2Flanggraph&color=yellow&logo=github&logoColor=white&style=flat&cacheSeconds=300)
...
- [Microsoft Agent Framework — Agent Harness](https://learn.microsoft.com/en-us/agent-framework/) - 🆕 **Build 2026 (June 2026)**. New production patterns shipped with MAF 1.0+: built-in automatic context management, human-in-the-loop approval gates, and GitHub Copilot SDK integration. Convergence of AutoGen + Semantic Kernel at enterprise scale.
- [Google ADK 2.0](https://github.com/google/adk-python) - 🆕 **GA June 2026**. Google's Agent Development Kit hits 2.0 with a graph-based orchestration system, unified Agent2Agent (A2A) Protocol support, managed Agents API, and Gemini 3.5 Flash integration. Four-layer stack for building production-ready agents on Google Cloud. ![GitHub stars](https://img.shields.io/badge/dynamic/json?label=Stars&query=%24.stargazers_count&url=https%3A%2F%2Fapi.github.com%2Frepos%2Fgoogle%2Fadk-python&color=yellow&logo=github&logoColor=white&style=flat&cacheSeconds=300)
- [Strands Agents (AWS)](https://github.com/strands-agents/sdk-python) - 🆕 **April–June 2026**. AWS open-source model-driven agent SDK (Python + TypeScript 1.0 GA April 30, 2026). Model-agnostic (Bedrock, Anthropic, OpenAI, Ollama), multi-agent orchestration patterns (graph/swarm/workflow), built-in observability hooks, A2A Protocol support, browser-side execution in TypeScript. 25M+ downloads. Apache-2.0. ![GitHub stars](https://img.shields.io/badge/dynamic/json?label=Stars&query=%24.stargazers_count&url=https%3A%2F%2Fapi.github.com%2Frepos%2Fstrands-agents%2Fsdk-python&color=yellow&logo=github&logoColor=white&style=flat&cacheSeconds=300)
- [CrewAI 1.14](https://github.com/crewAIInc/crewAI) - 🆕 **June 2026**. Major update ships pluggable backends for memory, knowledge, and RAG; declarative Flows with CLI/TUI support; Chat API for conversational agent flows; native Snowflake Cortex LLM provider; removed LangChain dependency for a lighter core. **1.15.2 (July 8)**: Flow Definitions, inline skill definitions, dynamic LLM model discovery, enhanced streaming and orchestration. **1.15.3 (July 16)**: step & execution-boundary interception, generic hook dispatcher, opt-in tool-result caching — "control-surface" release. ![GitHub stars](https://img.shields.io/badge/dynamic/json?label=Stars&query=%24.stargazers_count&url=https%3A%2F%2Fapi.github.com%2Frepos%2FcrewAIInc%2FcrewAI&color=yellow&logo=github&logoColor=white&style=flat&cacheSeconds=300)
- [Oracle AI Agent Studio (Fusion)](https://www.oracle.com/artificial-intelligence/ai-agent-studio/) - 🆕 **July 14, 2026**. AI-native builder within Oracle Fusion Cloud Applications for creating Fusion Agentic Applications — outcome-driven systems powered by teams of specialized agents that reason and execute within Fusion's business objects, workflows, and security context. No-code / low-code / pro-code options; included at no additional cost to Fusion customers.
...
- [Zep](https://github.com/getzep/zep) - Long-term memory for AI assistants and agents. **July 2026**: new enterprise docs on Attribute-Based Access Control (ABAC) and episode metadata projection; enhanced production readiness for regulated industries. ![GitHub stars](https://img.shields.io/badge/dynamic/json?label=Stars&query=%24.stargazers_count&url=https%3A%2F%2Fapi.github.com%2Frepos%2Fgetzep%2Fzep&color=yellow&logo=github&logoColor=white&style=flat&cacheSeconds=300)
...
- [AgentServices](https://agentservices.to) - 🆕 ⚠️ **Unverified.** Paid data APIs for AI agents with x402 on-chain payments — 54 services covering crypto market data, financial intelligence, onchain analytics, and inference. MCP server with 37 tools; agents pay per-call in USDC on Base via HTTP 402. Early-stage single-maintainer project; evaluate before depending on it.
...
- [Google Antigravity Sandbox](https://antigravity.google/blog/introducing-google-antigravity-2-0) - 🆕 **June 2026**. Sandboxed Linux environments for agent-executed code; ships as part of Antigravity 2.0's four-layer stack — dynamic sub-agents each run in their own isolated container with scoped filesystem + network access.
...
- [Microsoft Prompt Shields](https://www.microsoft.com/en-us/security/blog/) - 🆕 **June 2026**. Azure AI Content Safety feature specifically designed to detect and neutralize indirect prompt injection attacks — where malicious instructions are hidden in documents or web pages an agent consumes. Integrates with Azure OpenAI Service and third-party models.
...
- [Lineation.ai](https://lineation.ai) - 🆕 **July 2026**. Agentic security platform with zero-trust control plane and lightweight endpoint daemon. Assigns Zero Trust Non-Human Identities (NHI) to each agent; prevents goal hijacking, memory poisoning, and tool misuse; immutable reasoning audit trail for SOC 2 / HIPAA / EU AI Act compliance; validates MCP data handshakes in real-time. Cloud (free-to-start) and on-prem.
...
- [CrowdStrike Falcon AIDR](https://www.crowdstrike.com/) - 🆕 **July 2026**. AI Detection and Response — new security category pioneered by CrowdStrike. Creates a live inventory of all AI agents running across an enterprise, assigns risk scores, detects behavioral anomalies in real time, and provides automated response for both workforce AI adoption and enterprise-developed agents.
- [Exabeam Agent Behavior Intelligence](https://www.exabeam.com/) - 🆕 **July 2026**. Extension of Exabeam’s Behavior Intelligence platform to cover agentic AI risks. Adds open-source agent-verification and telemetry projects to standardize security practices — moving from static guardrails to continuous verify-observe-analyze-improve loops.
- [AI AGENT Act (US Discussion Draft)](https://dwt.com/) - 🆕 **Late June 2026 (Regulatory Reference)**. Proposed US federal framework — Artificial Intelligence Access, Gatekeeper Exchange, and Nondiscriminatory Transfer Act — that would require large online platforms to allow users to delegate access rights to authorized third-party AI agents via interoperable interfaces. Not yet law; listed for awareness.
- [ITU AI Agent Trust Focus Group](https://itu.int/) - 🆕 **July 2026**. UN agency ITU launches a new focus group to develop common terminology, reference architectures, trust frameworks, and security criteria for the continuous assessment of AI agents throughout their lifecycle.
...
- [Codex CLI](https://github.com/openai/codex) - OpenAI's open-source terminal coding agent (Rust, Apache-2.0, 82K+ stars). 77.3% Terminal-Bench score. May 2026 adds **Codex Chrome extension** for in-browser DevTools workflows, `codex remote-control` headless app-server, plugin-detail bundled-hook display, and **Codex on Mobile** preview (May 14) that lets ChatGPT iOS/Android remote-control the macOS Codex app. **July 9, 2026**: Codex fully integrated into the **ChatGPT desktop app** (no longer a standalone tool); **Multi-Agent V2** protocol ships with enhanced inter-agent communication; frequent CLI point updates throughout July. **Codex Micro** ($230 keypad for Codex control, pre-order ships July 24) also announced. ![GitHub stars](https://img.shields.io/badge/dynamic/json?label=Stars&query=%24.stargazers_count&url=https%3A%2F%2Fapi.github.com%2Frepos%2Fopenai%2Fcodex&color=yellow&logo=github&logoColor=white&style=flat&cacheSeconds=300)
...
- [OpenCode](https://github.com/opencode-ai/opencode) - 📦 **Archived**. Open-source terminal-first coding agent with a native TUI, supported OpenAI, Claude, Gemini, Ollama (local models), and LSP for code intelligence. Multi-session, model-agnostic, Go-based. MIT. ![GitHub stars](https://img.shields.io/badge/dynamic/json?label=Stars&query=%24.stargazers_count&url=https%3A%2F%2Fapi.github.com%2Frepos%2Fopencode-ai%2Fopencode&color=yellow&logo=github&logoColor=white&style=flat&cacheSeconds=300)
...
- [Claude Agent SDK](https://docs.anthropic.com/en/docs/claude-code/sdk) - 🆕 **June 2026**. Anthropic rebrands the Claude Code SDK and ships major agentic features: hierarchical subagent spawning (up to 5 levels deep), Dynamic Workflows (fan out tens–hundreds of parallel subagents), Performance Outcomes (grader agent reviewing sub-agent outputs until criteria met), and fallback model chains. Powers the "ultracode" mode enabling up to 1,000 subagents per execution. Python + TypeScript.
...
- [ENPIRE](https://windflash.us/daily-report/en/2026-06-22) - 🆕 **June 2026**. NVIDIA/CMU/UC Berkeley framework enabling AI agents to conduct their own robotics research — managing dual-arm robots, modifying algorithms, and training policies without human intervention.
- [Kairos](https://futureiot.tech/kairos-model-breaks-new-ground-for-open-source-embodied-intelligence/) - 🆕 **June 2026**. ACE ROBOTICS open-sources its top-ranking vision-language-action (VLA) world model.
...
- [Mitsubishi "N" Humanoid](https://www.mitsubishi.com/) - 🆕 **July 2026**. Mitsubishi reveals plans to develop and mass-produce its own humanoid robot, named **"N"**, in partnership with Highlanders. Targeting 1,000 units/month by 2027 to address Japan's acute labor shortage in manufacturing and logistics.
...
- [WebArena](https://webarena.dev/) - Realistic, reproducible web environment (Reddit / shopping / GitLab clones) used by OSWorld and most browser-agent papers. **July 2026 — WebArena-Verified**: maintainers released a manually-reviewed version correcting all 812 tasks, reference answers, and evaluators; replaces non-deterministic judging with deterministic type-aware checks. July 15, 2026 leaderboard top: **Muse Spark 1.1 at 69%** (first publicly displayed score). A "Hard" subset of 258 tasks also available.
...
- [Terminal-Bench 2.1](https://www.tbench.ai/) - 🆕 **June 2026**. Updated Terminal-Bench with expanded CLI task coverage; June 18, 2026 leaderboard: Codex CLI + GPT-5.5 at 83.4%, Claude Code + Fable 5 at 83.1%. Gap between top CLI agents now under 1%.
...
- [Agents' Last Exam (ALE)](https://arxiv.org/abs/2606.05405) - 🆕 **June 2026**. UC Berkeley (RDI) + Snorkel AI benchmark evaluating agents on 1,500+ long-horizon, economically valuable tasks across 55 professional subfields — created with 300+ industry experts. Frontier models score <1% full-pass on the hardest tier; living benchmark with task rotation every 6 months. arXiv:2606.05405.
- [JetBrains Kotlin Benchmark](https://blog.jetbrains.com/kotlin/2026/07/kotlin-benchmark-for-ai-coding-agents/) - 🆕 **July 2026**. Public benchmark from JetBrains measuring AI coding agents on real-world Kotlin development tasks — code generation, refactoring, test writing, and bug fixing. Gives developers a credible, language-specific evaluation beyond general SWE-bench scores.
- [Stripe Agent Benchmark](https://stripe.com/blog/ai-agent-benchmark-2026) - 🆕 **July 2026**. End-to-end evaluation suite testing whether AI agents can build complete Stripe integrations from scratch — covering code generation, test execution, and production-style validation. Claude Opus 4.5 achieved 92% average score on full-stack API integration tasks in initial evaluations.
- [Agents' Last Exam (ALE) — July Update](https://openai.com/blog/gpt-5-6) - 🆕 **July 9, 2026**. Highlighted by OpenAI at GPT-5.6 launch. 1,500+ expert-sourced professional-workflow tasks across 55 non-physical occupations. July 2026 leaderboard: Codex + GPT-5.6 Sol achieves **53.6 / 100**; gap vs previous top models ~12 points.
...
- [GDPval Benchmark](https://openai.com/blog/gpt-5-6) - 🆕 **July 2026** (expanded at GPT-5.6 launch). Spans 44 knowledge-work occupations selected from top U.S. GDP-contributing industries; evaluates AI’s ability to assist professionals with day-to-day knowledge tasks. Companion to ALE.
...
- [Vercel Agent Browser](https://github.com/vercel-labs/agent-browser) - 🆕 Headless browser automation CLI for AI agents. June 2026 release adds `vitals` command for Core Web Vitals (LCP/CLS/TTFB/INP), `pushstate` for SPA navigation, out-of-process plugin system, MCP server mode, and `@agent-browser/sandbox` for hosted environments. Apache-2.0, 37K+ stars. ![GitHub stars](https://img.shields.io/badge/dynamic/json?label=Stars&query=%24.stargazers_count&url=https%3A%2F%2Fapi.github.com%2Frepos%2Fvercel-labs%2Fagent-browser&color=yellow&logo=github&logoColor=white&style=flat&cacheSeconds=300)
- [Google ADK 2.0 Agent Browser Integration](https://github.com/google/adk-python) - 🆕 **June 2026**. Google's Agent Development Kit 2.0 (GA) ships graph-based orchestration and A2A Protocol integration; now includes native browser-agent primitives as part of the four-layer AI agent stack unveiled post-I/O 2026. ![GitHub stars](https://img.shields.io/badge/dynamic/json?label=Stars&query=%24.stargazers_count&url=https%3A%2F%2Fapi.github.com%2Frepos%2Fgoogle%2Fadk-python&color=yellow&logo=github&logoColor=white&style=flat&cacheSeconds=300)
...
- [Muse Spark 1.1](https://muse.ai/) - 🆕 **July 15, 2026**. New web agent topping WebArena-Verified at **69%** — first publicly scored agent on the verified version of the benchmark (812 manually-reviewed tasks with deterministic evaluators). Closed-source; API access through Muse AI platform.
- [Firecrawl v2](https://github.com/mendableai/firecrawl) - 🆕 **July 2026**. Major update to the agentic web scraping platform: **Deep Research mode** (autonomous multi-hop crawl + synthesis into a structured report), improved JavaScript rendering, live crawl webhooks, and batch URL processing. Used by 50K+ developers. Apache-2.0. ![GitHub stars](https://img.shields.io/badge/dynamic/json?label=Stars&query=%24.stargazers_count&url=https%3A%2F%2Fapi.github.com%2Frepos%2Fmendableai%2Ffirecrawl&color=yellow&logo=github&logoColor=white&style=flat&cacheSeconds=300)
...
- [AgentArk](https://github.com/letsdatascience/agentark) - 🆕 **June 2026**. Personal AI OS prioritizing local control and security. Features GEPA (Generative Evolution of Prompts and Actions) for self-learning from execution traces. ![GitHub stars](https://img.shields.io/badge/dynamic/json?label=Stars&query=%24.stargazers_count&url=https%3A%2F%2Fapi.github.com%2Frepos%2Fletsdatascience%2Fagentark&color=yellow&logo=github&logoColor=white&style=flat&cacheSeconds=300)
- [nanobot](https://github.com/HKUDS/nanobot) - 🆕 Lightweight open-source personal AI agent. "Durability Release" (v0.2.2, June 2026) added segmented WebUI transcripts, first-class Python SDK runtime controls, and Firecrawl integration for daily long-running tasks. ![GitHub stars](https://img.shields.io/badge/dynamic/json?label=Stars&query=%24.stargazers_count&url=https%3A%2F%2Fapi.github.com%2Frepos%2FHKUDS%2Fnanobot&color=yellow&logo=github&logoColor=white&style=flat&cacheSeconds=300)
...
- [Databricks Genie One](https://www.databricks.com/blog/introducing-genie-one-genie-ontology-and-genie-agents) - 🆕 **June 16, 2026 (Data + AI Summit)**. Agentic "data coworker" that automates and orchestrates work across structured and unstructured data — inside or outside Databricks — grounded in the new **Genie Ontology** (an organization-wide knowledge graph) and governed by Unity Catalog. Ships with Genie Agents and a Genie App Builder; Databricks reports 84.5% first-attempt accuracy in internal testing.
...
- [LangSmith Engine](https://www.langchain.com/blog/interrupt-2026-overview) - 🆕 **May 2026 (Interrupt 2026)**. Autonomous failure-diagnosis layer for LangSmith — clusters production failures into prioritised issues, root-causes them across traces and code, and proposes fixes for human review. Companion to the new SmithDB (Rust + DataFusion-backed agent observability database). **July 6-10, 2026**: one-click Slack integration to add any agent directly to Slack (agents can request tool approvals inside Slack); improved playground / prompt management; monthly trace-count limits per project/user for enterprise admins.
...
- [Prismix](https://prismix.dev) - Real-time status monitoring for 75+ AI services (OpenAI, Anthropic, Cursor, etc.) with email/webhook alerts, AI news aggregator from 70+ sources, and MCP server directory with 500+ servers.

---

- [Ceros](https://www.prnewswire.com/news-releases/ceros-launches-providing-unified-identity-observability-and-governance-for-every-ai-agent-and-workflow-302800721.html) - 🆕 **June 2026**. Agentic AI trust layer for unified identity, observability, and governance. Solves the "silent failure" problem in non-deterministic systems with complete trace visibility across tool calls and sub-agent handoffs.
...
| **Jul 16, 2026** | [Kimi K3](https://kimi.ai/) launched by Moonshot AI — 2.8T parameter sparse MoE (896 experts, 16 active), 1M-token context, KDA + AttnRes architecture, $0.30/$3.00/$15.00 per million tokens; full open weights promised July 27 | Models |
| **Jul 16, 2026** | [CrewAI 1.15.3](https://github.com/crewAIInc/crewAI) — "control-surface" release: step/execution-boundary interception, generic hook dispatcher, opt-in tool-result caching | Frameworks |
| **Jul 10, 2026** | [LangGraph 1.2.9](https://github.com/langchain-ai/langgraph) released — continued production hardening of the stateful multi-agent runtime | Frameworks |
| **Jul 10, 2026** | [Cursor 3.11](https://cursor.com/changelog) — Side Chats, conversation history search, Cloud Agent Hooks for granular agent observability; Grok 4.5 remains default model | Tools |
| **Jul 9, 2026** | GPT-5.6 Sol / Terra / Luna GA on ChatGPT, Codex, and API; Codex integrated into ChatGPT desktop app; Codex Multi-Agent V2 ships | Models |
...
| **Jul 8, 2026** | Grok 4.5 released by xAI — joint-trained with Cursor; 500K context, $2/$6 per million tokens; default model in Cursor | Models |
| **Jul 8, 2026** | OpenAI audit of SWE-bench Pro reveals ~30% of tasks broken; OpenAI retracts recommendation to use it as a primary coding-agent metric | Research |
| **Jul 7, 2026** | Arize Phoenix July 7 release: Metric Charts, Trace Search, expanded REST API | Tools |
| **Jul 7, 2026** | Meta Muse Image launched — agentic image generation integrated into Instagram Stories (US) + WhatsApp DMs | Models |
...
| **Jul 1, 2026** | Claude Fable 5 global reinstatement — US Commerce Dept lifts export controls; Anthropic adds new safety filter | Models |
| **Jul 8, 2026** | [CrewAI 1.15.2](https://github.com/crewAIInc/crewAI) — Flow Definitions with inline skill definitions, dynamic LLM model discovery, enhanced streaming and orchestration | Frameworks |
...
*540+ resources across 26 categories — from foundation models to agent protocols to generative AI.*
