...
- [Hypha](https://github.com/CodeSoul-co/Hypha) - 🆕 ⚠️ **Unverified** (v1.0.1, August 14, 2026; Apache-2.0). TypeScript agent framework from CodeSoul that separates an **Agent Core** (ReAct, planning, tool selection, memory) from a **Production Harness** (FSM execution, policy/approval, checkpoints, recovery, replay, audit); product behaviour is declared as versioned **DomainPacks**, and a typed cache plane is explicitly barred from authorising side effects or advancing the FSM. 15 `@codesoul-co/hypha-*` packages on npm. ⚠️ Treat the star count with caution: the vendor's Skill Garden gates private Skill downloads behind starring this repo, npm downloads were ~32/month as of 2026-08-22, and the vendor-published τ³ result (0.636 vs 0.626 direct-model baseline over 385 single-trial tasks) is inside statistical noise. ![GitHub stars](https://img.shields.io/badge/dynamic/json?label=Stars&query=%24.stargazers_count&url=https%3A%2F%2Fapi.github.com%2Frepos%2FCodeSoul-co%2FHypha&color=yellow&logo=github&logoColor=white&style=flat&cacheSeconds=300)
