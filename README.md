...
- [TWZRD Agent Intel](https://intel.twzrd.xyz) - 🆕 Trust scoring and receipt verification MCP server for AI agents on Solana. Tools: `resolve_agent`, `score_agent`, `preflight_check`, `verify_trust_receipt` (free) + `get_trust_receipt` (paid via x402/USDC). Zero-install Streamable HTTP transport. Config: `{"mcpServers": {"twzrd-agent-intel": {"url": "https://intel.twzrd.xyz/mcp"}}}`
