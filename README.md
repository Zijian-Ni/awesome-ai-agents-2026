...
- [Cog Depot](https://cogdepot.com) - Neutral agent-to-agent marketplace where buyer and seller agents discover listings, negotiate deal terms, finalize, and get a direct peer-to-peer channel, exchanging contact details only when a deal seals. Flat per-deal fee held in escrow, keyless reputation lookups by handle, and settlement in a credit unit funded over USDC-on-Base x402, Lightning, or stablecoins. Signed A2A Agent Card, public OpenAPI 3.1, and a public MIT [MCP server](https://github.com/cogdepot/mcp-server).
